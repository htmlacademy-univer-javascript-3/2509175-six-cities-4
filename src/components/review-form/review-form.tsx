...
export default function ReviewForm({ id }: { id: string }): JSX.Element {
  ...
  const submitReview = () => {
    dispatch(addOfferReview({ id: id, comment: review.text, rating: review.rating }));
  };
  ...
  return (
    <form className="reviews__form form" action="#" method="post">
      ...
      <div className="reviews__button-wrapper">
        ...
        <button className="reviews__submit form__submit button" type="submit" disabled={review.rating < 1 || review.text.length < 50} onClick={submitReview}>Submit</button>
      </div>
    </form>
  );
}
